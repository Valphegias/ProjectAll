...
///find all Vp point
int32 Player::GetPoints()
{
QueryResult* result = LoginDatabase.PQuery("SELECT `points` FROM `account` WHERE `id` = '%u'", GetSession()->GetAccountId());
if(!result)
return false;
Field *fields = result->Fetch();
int32 nbr_points = fields->GetInt32();
return nbr_points;
}
